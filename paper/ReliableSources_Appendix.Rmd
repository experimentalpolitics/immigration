...
plot_grid(
  ...
  ggplot(df, aes(x = Condition, fill = Sex)) +
...
m_balance <- multinom(Condition ~ age + income + male + usborn + college + white + 
                        pid_rep + ideol_con + lazy_hispanics + problem_immigration, 
                      data = df)
```
...
stargazer(m_balance,
          ...
          title = "Balance tests for treatment conditions (vs. control condition as reference). Coefficients are based on multinomial logit model. Positive coefficients indicate larger probability of treatment assignment. Standard errors in parentheses.", 
          table.placement = "ht",
          label = "tab:balance",
