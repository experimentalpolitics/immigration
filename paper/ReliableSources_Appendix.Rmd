...
```{r balance_plot, fig.height=8.5, fig.width=7}
df <- df %>% mutate(
  Condition = recode_factor(condition,
                            `control` = "Control",
                            `assigned` = "Forced exposure",
...
stargazer(m_balance,
          ...
          title = "Balance tests for treatment conditions (vs. control group as reference category). Coefficients are based on multinomial logit model. Positive coefficients indicate larger probability of treatment assignment. Standard errors in parentheses.", 
          table.placement = "ht",
          label = "tab:balance",
...
```{r preference, fig.height=4, fig.width=3.85}
df %>%
  filter(!is.na(prefer)) %>%
...
While we didn't ask respondents directly whether they consider the article to be authentic (in order to avoid priming respondents accordingly), we asked participants to evaluate the news article on various dimensions such as "good vs. bad" or "accurate vs. inaccurate" (see Appendix A IV). The response patterns conditional on the article source and the participants' media preference suggest that they viewed the ascribed publisher as credible.

: In particular, respondents who prefer MSNBC evaluated the article more negatively if it was published by Fox News, while we observe a reverse pattern for respondents who prefer Fox News (although the differences here do not reach conventional levels of statistical significance).
