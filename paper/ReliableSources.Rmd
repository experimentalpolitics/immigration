...
```{r m1, fig.height=2, fig.width=7, fig.cap="\\label{fig:m1}M1"}
...
```{r m2, fig.height=2, fig.width=7, fig.cap="\\label{fig:m2}M2"}
df_nochoice <- df %>%
...
m2robust %>%
  ...
  geom_point(position=position_dodge(width=-0.4)) +
  geom_errorbar(width=0, position=position_dodge(width=-0.4)) +
  theme_bw(base_size = 8) + labs(x = NULL, y = "Coefficient") +
  facet_wrap(~group, scales = "free_x", ncol = 3) +
  scale_colour_brewer(palette = "Set1")
```
...
```{r m3, fig.height=2, fig.width=7, fig.cap="\\label{fig:m3}M3"}
df_assigned <- df %>%
...
ggplot(m3, aes(y = diff, ymin = cilo, ymax = cihi, x = outcome,
  ...
  geom_point(position=position_dodge(width=-0.4)) +
  geom_errorbar(width=0, position=position_dodge(width=-0.4)) +
  theme_bw(base_size = 8) + labs(x = NULL, y = "Treatment effect of reading\nFox rather than MSNBC") +
  facet_wrap(~group, scales = "free_x", ncol=3) +
  scale_colour_brewer(palette = "Set1")
```


```{r m4, fig.height=2.7, fig.width=7, fig.cap="\\label{fig:m4}M4"}
m4 <- bind_rows(
