...
```{r m1, fig.height=4.5, fig.width=4.5}
...
```{r m2, fig.height=4.5, fig.width=4.5}
df_nochoice <- df %>%
...
```{r m3, fig.height=4.5, fig.width=7}
df_assigned <- df %>%
  ...
  mutate(prefer = 
         ...
         eval = wp_fair + wp_american + wp_accurate - wp_hostile - wp_bad - wp_skewed,
         share = actions_discuss + actions_forward + actions_post) %>%
  split(.$prefer)
