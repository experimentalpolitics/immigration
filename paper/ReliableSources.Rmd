...
<!--OR is it motivation/open-mindedness? Are they the same thing?? ex. the initial decision to access a certain source may compel people to be more open to accepting the information provided therein.<!--so is motivation the mechanism? Whether people adopt these interpretations along with the factual information may be partly driven by people's initial motivation to access the information---presumably more so than when considering factual information alone.<!--per comments from JJ: unclear why the last point would be true--do we have cites for this assertion?--><!--the underlying mechanism is still unclear -->
...
We contend that it is ultimately an empirical question whether the effect of the ability to choose media sources on opinion/attitude change is solely driven by media preferences and related source credibility. However,our empirical analyses---to which we are going to turn next---provide at least some evidence that this may not be the case. Instead, the source of the information matters, as do the circumstances of exposure (and these effects vary across stages of opinion formation). <!--so, there's a conditional relationship here? consistent/inconsistent source * forced/free choice --->

<!--why would opinion change only occur among those allowed to choose their news source? -->

<!--differentiate H2/H3 by discussing how we test each of these. that way, the reader can better see the difference.If H2 is the straightforward effect of assignment to free choice/forced, then what's the quantity of interest for H3? H3 is the difference between those recieving a consistent/inconsistent source. how we distinguish between consistent/inconsistent: Based on an item in the pre-test battery, we can distinguish whether participants in the treatment conditions were exposed to an information source that is consistent or inconsistent with their usual media preferences. But we also go farther--add some discussion of what it looks like in practice--so, even among those who recieve information from consistent sources, we expect there's still a difference between forced/choice. -->  <!--Still there's unclearness between H2 and H3, Amanda's sugesstion could be one solution Or...--> 


<!--where to introduce the ambivalence piece?--> 
