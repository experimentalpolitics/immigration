...
```{r m1, fig.height=2, fig.width=7, fig.cap="\\label{fig:m1}Treatment effects of forced exposure and free choice condition. Coefficients are based on linear regression models controlling for pre-treatment covariates. Full model results included in the appendix."}
p1
```

```{r m2, fig.height=3.5, fig.width=7.1, fig.cap="\\label{fig:m2}Treatment effects conditional on media preference. Coefficients are based on linear regression models controlling for pre-treatment covariates. Full model results included in the appendix."}
p2
```


```{r m4, fig.height=2, fig.width=6.5, fig.cap="\\label{fig:m4} Voluntary vs. involuntary exposure to inconsistent sources"}
p4
```
