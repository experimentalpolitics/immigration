...
```{r Hypotheses 1a-c}
df %>%
  select(condition, employ_correct, sales_correct, 
  ...
  mutate(group = recode_factor(variable, 
                               ...
                               .ordered = TRUE),
         outcome = recode_factor(variable, 
                                 ...
                                 .ordered = TRUE)) %>%
  ggplot(aes(y = mean, ymin = cilo, ymax = cihi, x = outcome,
             shape = condition, col = condition)) + 
  theme_bw() + labs(x = "Outcome", y = "Average Value") +
...
```{r OLS model}
df$libimm <- rowMeans(subset(df, select = c(immig_increased, taxes_pos, jobs_pos)), na.rm = TRUE)

lm(libimm ~ condition + age + male + usborn + white + college + income + marital + church, data = df) %>% summary()

lm(libimm ~ condition, data = df) %>% summary()

h1a <- lm(libimm ~ condition, data = df)

h1b <- lm(sales_correct ~ condition, data = df)

h1c <- lm(employ_correct ~ condition, data = df)
