...
> `We added a discussion of our modeling choices in the results section and explained why we implemented a linear probability model instead of a logit/probit when the outcome vatiable is dichotomous (now on p. 13). In addition, we replicated all analyses with dichotomous outcome measures in the paper using logistic regression (see Appendix B.III). The substantive results are consistent across model specifications.`
