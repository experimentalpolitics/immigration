...
We consider several outcome measures to capture immigration attitudes and how the news story is received (see full questions below). Unless otherwise specified, outcomes that are measured using multiple variables will be combined in a additive scale. We are going to ensure that all items load properly on a single factor and exclude those that do not. Furthermore, we will report results using individual items in the appendix.

**Economic impact of immigration:** Our first outcome of interest addresses the perceived economic impact of legal immigration. The news article includes specific information on the number of people in the US who are employed by immigrant-owned businesses as well as the total value of sales of immigrant-owned businesses. Substantively, we want to examine whether the news article is able to correct common factual misperceptions on these issues.

**Attitudes towards immigration:** Our second outcome measure focuses on people's attitudes towards legal immigration. We include one question asking participants whether the number of immigrants should be increased or decreased (this item will be analyzed individually). Two additional questions capture respondents' beliefs about how much immigrants contribute to society by paying taxes and creating jobs. Together, the items will allow us to assess whether the correction of misperceptions is associated with subsequent attitude change.

**Trust in news sources:** Lastly, we include a battery of items measuring trust in different news sources. Since the content of the news article is held constant between sources, we can examine how exposure to an atypical news story (in this case published by FoxNews) impacts people's perception of the source itself.
...
**Engagement with tweet:** As described above, the tweet shown to participants consists of a link to a news article. While the link itself is not active, we can capture whether participants nevertheless try to click on it.  Since participants are never asked explicitly to click on any links, their willingness to do so indicates the degree to which they are interested in further engaging with the content voluntarily.

**Response latencies:** In order to further assess the extent to which respondents are interested in the article and comply with the instructions to read it, we will also measure the amount of time that individuals spend viewing the tweet and reading the news story.
...
For the main outcome measures (i.e., those that are also included in the control group), our analysis will consist of three comparisons:

1. **control vs. assigned source:** Those who receive information from an assigned source are more likely to update their beliefs than those who are in the control group.
2. **control vs. free choice:** Those who receive information from a chosen sources are more likely to update beliefs than those in the control group.
3. **(control vs. assigned source) vs. (control vs. free choice):** [which is essentially equivalent to assigned vs. free choice]
