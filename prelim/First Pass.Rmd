...
Control vs Free Choice - I intially plotted them together on the same line (3 controls in one column; 4 choice in another and 2 no preference), but it was impossible to read. Maybe there is a different way to do that, but this should give us a decent first look!

```{r}
data <- data.frame(x = c("Control, Prefer M", "Control, Prefer F", "Control, Prefer N", "Free Choice Same, Prefer M", "Free Choice Same, Prefer F", "Free Choice Diff, Prefer M", "Free Choice Diff, Prefer F", "No Preference, Chose MSNBC", "No Preference, Chose FOX"),
                   ...
                   CHI       = c(0.052, 0.069, 0.047, 0.108, 0.07, 0.223, 0.067, 0.092, 0.09),
                   CLO = c(0.052, 0.069, 0.047, 0.108, 0.07, 0.223, 0.067, 0.092, 0.09),
                   Preference = c("Prefer M", "Prefer F", "Prefer N", "Prefer M", "Prefer F", "Prefer M", "Prefer F", "Prefer N", "Prefer N"),
                   Random_Group = c("Control", "Control", "Control", "Choice", "Choice", "Choice", "Choice", "Choice", "Choice"))
...
Choice <- ggplot(data, aes(x, y, group = Preference)) +
  ...
  labs(x = "",
       ...
       title = "FREE CHOICE") +
  theme_classic() + 
  geom_point(aes(color=Preference, shape=Random_Group, size = Random_Group)) + 
  scale_shape_manual(values=c(15, 17, 16)) +
  scale_size_manual(values=c(4,4,4)) +
  theme_gray(base_size = 14) + 
  theme(axis.text.x = element_text(angle = 45, size = 8)) + scale_x_discrete(limits = data$x)
Choice
```
...
Control vs Assigned - I intially plotted them together on the same line (3 controls in one column; 4 assigned in another and 2 no preference), but it was impossible to read. Maybe there is a different way to do that, but this should give us a decent first look!

```{r}
data2 <- data.frame(x2 = c("Control, Prefer M", "Control, Prefer F", "Control, Prefer N", "Assigned Same, Prefer M", "Assigned Same, Prefer F", "Assigned Diff, Prefer M", "Assigned Diff, Prefer F", "No Preference, Assigned MSNBC", "No Preference, Assigned FOX"),
                    ...
                    CHI_2      = c(0.052, 0.069, 0.047, 0.067, 0.084, 0.092, 0.114, 0.085, 0.066),
                    CLO_2 = c(0.052, 0.069, 0.047, 0.067, 0.084, 0.092, 0.114, 0.085, 0.066),
                    Preference2 = c("Prefer M", "Prefer F", "Prefer N", "Prefer M", "Prefer F", "Prefer M", "Prefer F", "Prefer N", "Prefer N"),
                    Random_Group_2 = c("Control", "Control", "Control", "Assigned", "Assigned", "Assigned", "Assigned", "Assigned", "Assigned"))
...
Assigned <- ggplot(data2, aes(x2, y2, group = Preference2)) +
  ...
  labs(x = "",
       ...
       title = "ASSIGNED") +
  theme_classic() + 
  geom_point(aes(color=Preference2, shape=Random_Group_2, size = Random_Group_2)) + 
  scale_shape_manual(values=c(15, 17, 16)) +
  scale_size_manual(values=c(4,4,4)) +
  theme_gray(base_size = 14) + 
  theme(axis.text.x = element_text(angle = 45, size = 8)) +
  scale_x_discrete(limits = data2$x2)
Assigned
```